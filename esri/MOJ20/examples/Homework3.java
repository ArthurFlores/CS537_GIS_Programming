...
This assignment makes shape files out of selected layers
for points, polylines, and polygons.
When the shape file is being made the new layer
flashes on the screen. When a new point layer is made from
a selection it will bring up a new icon for the point.
When there is a new polyline layer from a selection
then the lines are made dashed and green.
...
public class Homework3 extends JFrame {
  ...
  Toc toc = new Toc();
  String s1 = "C:\\ESRI\\MOJ20\\Samples\\Data\\CanadaSim\\province.shp";
  String s2 = "C:\\ESRI\\MOJ20\\Samples\\Data\\CanadaSim\\cities.shp";
  String s3 = "C:\\ESRI\\MOJ20\\Samples\\Data\\CanadaSim\\roads.shp";
  String datapathname = "";
  String legendname = "";
  ...
  public Homework3() {
    ...
    layerlis = new ActionListener() {public void actionPerformed(ActionEvent ae){
          ...
          lp.setVisible(true);
			...
			shT = flayer2.getLayerInfo().getType();

			com.esri.mo2.map.draw.BaseSimpleRenderer sbr = new com.esri.mo2.map.draw.BaseSimpleRenderer();
			com.esri.mo2.map.draw.SimplePolygonSymbol sfs = null;// for polygon
			com.esri.mo2.map.draw.RasterMarkerSymbol sms = null; // for points
			com.esri.mo2.map.draw.SimpleLineSymbol sls = null; // for lines
			//This sets the polygon layer and flashes yellow
			if (shT == 2) {
				sfs = new com.esri.mo2.map.draw.SimplePolygonSymbol();// for polygons
				sfs.setPaint(AoFillStyle.getPaint(com.esri.mo2.map.draw.AoFillStyle.SOLID_FILL,new java.awt.Color(0,0,0)));
				sfs.setBoundary(true);
				sbr.setSymbol(sfs);
			}
			//This sets the point layer and makes it a beat symbol and flashes the symbol
			else if (shT == 0) {
			...
			//This sets a line layer when selected makes a green and dashed shapefile
			else if (shT == 1) {
			    float dash[] = { 20.0f };
...
class CreateShapeDialog extends JDialog {
  ...
  //int shT = 0;
  int shT = Homework3.layer4.getLayerInfo().getType();
  ActionListener lis = new ActionListener() {public void actionPerformed(ActionEvent ae) {
        ...
        if (o == nameField) {
          ...
          path = "C:\\ESRI\\MOJ20\\EXAMPLES\\Homework3_ShapeFiles";  //changed path to a homework3 folder
      ...
      setVisible(false);
          ...
          try {
                ShapefileWriter.writeFeatureLayer(selectedlayer,path,name,shT);
